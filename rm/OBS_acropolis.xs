...

{
 ...
 // Close Objects
  ...
  // Med Settlement
   ...
   int setAvoidTower = rmCreateTypeDistanceConstraint("med Settlement avoid tower", "tower", 40.0);
   int setAvoidTree = rmCreateTypeDistanceConstraint("med Settlement avoid tree", "tree", 8.0);
   ...
   rmSetObjectDefMaxDistance(medSettlementID, 70.0);
...
/*      rmAddAreaCliffEdgeAvoidClass(id, classConnection, 1.0); */
  ...
  // Text
   ...
   // Gold
   PlaceDefNearPlayers(closeGoldID, true, 1);

   // Goats
   PlaceDefNearPlayers(closeGoatsID, true, rmRandInt(2,3));

   //Supa high poly chickens
   PlaceDefNearPlayers(closeChickensID, true, 1);
  ...
  // Text
   ...
   // Goats
   PlaceDefNearPlayers(mediumGoatsID, false, rmRandInt(0,2));
   ...
   // Goats.
   rmPlaceObjectDefAtLoc(farGoatsID, 0, 0.5, 0.5, cNonGaiaPlayers*rmRandInt(2, 4));

   // Berries.
   PlaceDefNearPlayers(farBerriesID, false, 2);
