...
void main(void)
{
   ...
   */
   ...
   // Elev.
   int numTries=6*cNonGaiaPlayers;
   int failCount=0;
   for(i=0; <numTries)
   {
   ...
   // Straggler trees.
   PlaceDefNearPlayers(stragglerTreeID, false, rmRandInt(2, 6));
   ...
   // cows
   PlaceDefNearPlayers(closeCowsID, true);
   ...
   if(bonusChance<0.7)
   PlaceDefNearPlayers(closeBoarID, false);
   ...
   PlaceDefNearPlayers(mediumWalrusID, false);

   PlaceDefNearPlayers(mediumDeerID, false, 1);
            
   // Far things.

   // Gold.
   PlaceDefNearPlayers(farGoldID, false, rmRandInt(1, 3));
   ...
   // Bonus huntable.
   PlaceDefNearPlayers(bonusHuntableID, false, 1);

   // Herds.
   PlaceDefNearPlayers(farCowID, false, rmRandInt(1,2));
   ...
   if(rmRandFloat(0,1)<0.75)
      PlaceDefNearPlayers(bonusWalrus, false, 1);
   else if(cNonGaiaPlayers < 8)
      PlaceDefNearPlayers(bonusWalrus, false, 2);
   else
      PlaceDefNearPlayers(bonusWalrus, false, 1);
   ...
   rmPlaceObjectDefAtLoc(fishID, 0, 0.5, 0.5, 5*cNonGaiaPlayers);
   ...
   rmPlaceObjectDefAtLoc(orcaID, 0, 0.5, 0.5, cNonGaiaPlayers*0.5);
