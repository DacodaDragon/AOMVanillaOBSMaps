...
/*      rmBuildArea(id); */
  ...
  // Text
   rmSetStatusText("",0.60);
     ...
     // Elev.
   int numTries=10*cNonGaiaPlayers;
   int failCount=0;
   for(i=0; <numTries)
   {
...
/*      rmSetAreaLocation(elevID, rmRandFloat(0.0, 1.0), rmRandFloat(0.0, 1.0)); */
   ...
   // Relics
   PlaceDefNearPlayers(relicID, false, 1);
  ...
  // Text
   ...
   // Random trees.
   rmPlaceObjectDefAtLoc(randomTreeID, 0, 0.5, 0.5, 7*cNonGaiaPlayers);
   rmPlaceObjectDefAtLoc(randomTreeID2, 0, 0.5, 0.5, 3*cNonGaiaPlayers);
   ...
   int forestCount=8*cNonGaiaPlayers;
   failCount=0;
   for(i=0; <forestCount)
   {
   ...
   rmPlaceObjectDefAtLoc(sandDrift, 0, 0.5, 0.5, 3*cNonGaiaPlayers);
