...
void main(void)
{
  ...
  // Text
   ...
   if(handedness<0.5)
   {
      sizel=2.5*sqrt(cNonGaiaPlayers*playerTiles);
      sizew=1.6*sqrt(cNonGaiaPlayers*playerTiles);
   }
   else
   {
      sizew=2.66*sqrt(cNonGaiaPlayers*playerTiles);
      sizel=1.5*sqrt(cNonGaiaPlayers*playerTiles);
   }
...
/*   rmPlacePlayersSquare(0.3, 0.1, 10.0); */
   ...
   float percentPerPlayer = 0.95/cNonGaiaPlayers;
   float teamSize = 0; 
   for(i=0; <cTeams)
   {
...
/*      rmSetAreaLocation(teamID, 0.3, 0.5); */
   ...
   rmBuildAllAreas();
    ...
    // Create player cores that can avoid water   for(i=1; <cPlayers)
/*   for(i=1; <cPlayers)
   {
...
/*      rmSetAreaLocPlayer(id2, i); */
   ...
   // Slight Elevation
   int failCount=0;
   int numTries=20*cNonGaiaPlayers;
   for(i=0; <numTries)
   {
  ...
  // Text
   ...
   // Straggler trees.
   PlaceDefNearPlayers(stragglerTreeID, false, 3);
  ...
  // Bonus huntable2
   ...
   // Berries.
   rmPlaceObjectDefAtLoc(farBerriesID, 0, 0.5, 0.5, cPlayers/2);
  ...
  rmPlaceObjectDefAtLoc(fishID, 0, 0.5, 0.5, 3*cNonGaiaPlayers); 
   ...
   rmPlaceObjectDefAtLoc(lilyID, 0, 0.5, 0.5, 4*cNonGaiaPlayers);
   ...
   rmPlaceObjectDefAtLoc(lily2ID, 0, 0.5, 0.5, 8*cNonGaiaPlayers);
  ...
  // Text
   ...
   rmPlaceObjectDefAtLoc(riverBushID, 0, 0.5, 0.5, 5*cNonGaiaPlayers);
