...
/*      rmSetAreaBaseHeight(smallPondID, 0.0); */
   ...
   // Close things....
   // TC
   PlaceDefNearPlayers(startingSettlementID, true);


   // Elev.
   int numTries=5*cNonGaiaPlayers;
   int failCount=0;
/*   for(i=0; <numTries)
   {
  ...
  // Text
   ...
   // Towers.
   PlaceDefNearPlayers(startingTowerID, true, 4);

   // Straggler trees.
   PlaceDefNearPlayers(stragglerTreeID, false, rmRandInt(1, 7));

   // Gold
   PlaceDefNearPlayers(startingGoldID, true); 

   // Goats
   PlaceDefNearPlayers(closeGoatsID, true);

   // Chickens or berries.
   PlaceDefNearPlayers(closeChickensID, true);

   // Close hunted
   PlaceDefNearPlayers(closeHuntableID, false);

   // Medium things....
   // Gold
   PlaceDefNearPlayers(mediumGoldID, false, rmRandInt(1, 2));

   // Deer
   PlaceDefNearPlayers(mediumDeerID, false);

   // Goats
   PlaceDefNearPlayers(mediumGoatsID, false, 2);

   // Far things.

   // Gold.
   PlaceDefNearPlayers(farGoldID, false, rmRandInt(2, 4)); 

   // Relics
   PlaceDefNearPlayers(relicID, false);
   ...
   if(rmRandFloat(0,1)<0.6)
      rmPlaceObjectDefAtLoc(farBerriesID, 0, 0.5, 0.5, cNonGaiaPlayers);

   // Bonus huntable stuff.
   rmPlaceObjectDefAtLoc(bonusHuntableID, 0, 0.5, 0.5, cNonGaiaPlayers);

   // Bonus huntable stuff.
   rmPlaceObjectDefAtLoc(bonusHuntable2ID, 0, 0.5, 0.5, cNonGaiaPlayers);

   // Predators
   PlaceDefNearPlayers(farPredatorID, false, 1);

   // Monkeys
   if(rmRandFloat(0,1)<0.5)
      PlaceDefNearPlayers(farMonkeyID, false, 1); 

   // Hawks
   PlaceDefNearPlayers(farhawkID, false, 2); 

   // Random trees.
   rmPlaceObjectDefAtLoc(randomTreeID, 0, 0.5, 0.5, 20*cNonGaiaPlayers);
  ...
  // Text
   ...
   failCount=0;
   //int numTries=30*cNonGaiaPlayers;
   //int maxCount=20*cNonGaiaPlayers;
   numTries=15*cNonGaiaPlayers;
   for(i=0; <numTries)
   {
   ...
   rmPlaceObjectDefAtLoc(bushID, 0, 0.5, 0.5, 10*cNonGaiaPlayers);
  ...
  // Text
   ...
   rmPlaceObjectDefAtLoc(deerID, 0, 0.5, 0.5, cNonGaiaPlayers);
   ...
   rmPlaceObjectDefAtLoc(deer2ID, 0, 0.5, 0.5, cNonGaiaPlayers);
   ...
   rmPlaceObjectDefAtLoc(rockID2, 0, 0.5, 0.5, 30*cNonGaiaPlayers);
