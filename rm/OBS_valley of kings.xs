...
void main(void)
{
   ...
   if(handedness<0.5)
   {
      sizel=2.22*sqrt(cNonGaiaPlayers*playerTiles);
      sizew=1.8*sqrt(cNonGaiaPlayers*playerTiles);
   }
   else
   {
      sizew=2.22*sqrt(cNonGaiaPlayers*playerTiles);
      sizel=1.8*sqrt(cNonGaiaPlayers*playerTiles);
   }
...
/*   rmPlaceObjectDefInRandomAreaOfClass(papyrusID, 0, smallPondID); */
   rmPlaceObjectDefAtLoc(papyrusID, 0, 0.5, 0.5, 6*cNonGaiaPlayers);
   ...
   // Straggler trees.
   PlaceDefNearPlayers(stragglerTreeID, false, rmRandInt(3,4));

   // Gold
   PlaceDefNearPlayers(startingGoldID, false);

   // Chickens
   PlaceDefNearPlayers(closeChickensID, false);      

   // Hippo.
   PlaceDefNearPlayers(closeHippoID, false);

   // Medium things....

   // Goats
   PlaceDefNearPlayers(mediumGoatsID, false);

   // Zebras
   PlaceDefNearPlayers(mediumZebraID, false);
   ...
   // Bonus huntable.
   PlaceDefNearPlayers(bonusHuntableID, false);
  
   // Bonus huntable.
   PlaceDefNearPlayers(bonusHuntableID2, false);
   
   // Goats.
   PlaceDefNearPlayers(farGoatsID, false, 2);

   // Predators
   PlaceDefNearPlayers(farPredatorID, false, 2);

    // Predators
   PlaceDefNearPlayers(farCrocsID, false);

   // Random trees.
   rmPlaceObjectDefAtLoc(randomTreeID, 0, 0.5, 0.5, 12*cNonGaiaPlayers);
   ...
   rmPlaceObjectDefAtLoc(skeletonID, 0, 0.5, 0.5, 8*cNonGaiaPlayers);
   ...
   rmPlaceObjectDefAtLoc(bushID, 0, 0.5, 0.5, 5*cNonGaiaPlayers);
   ...
   rmPlaceObjectDefAtLoc(bush2ID, 0, 0.5, 0.5, 3*cNonGaiaPlayers);
   ...
   rmPlaceObjectDefAtLoc(columnID, 0, 0.5, 0.5, 2*cNonGaiaPlayers);
   ...
   rmPlaceObjectDefAtLoc(baboonID, 0, 0.5, 0.5, cNonGaiaPlayers);
