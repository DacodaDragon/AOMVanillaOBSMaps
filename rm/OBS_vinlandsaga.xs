...
void main(void)
{
   ...
   int size=2.0*sqrt(cNonGaiaPlayers*playerTiles);
...
/*      rmSetAreaCoherence(id, 0.25); */
  ...
  // Cliff.
   ...
   // Settlements.
   rmPlaceObjectDefInArea(farSettlementID, 0, bonusIslandID, 2*cNonGaiaPlayers);

   // Gold
   PlaceDefNearPlayers(startingGoldID, false);

    // Berries  
   PlaceDefNearPlayers(closeBerriesID, true);

   // Straggler trees.
   PlaceDefNearPlayers(stragglerTreeID, false, rmRandInt(10,16));
   ...
   // Gold.
   rmPlaceObjectDefInArea(farGoldID, 0, bonusIslandID, 4*cNonGaiaPlayers);
   
   // Text
   rmSetStatusText("",0.60);

   // Relics
   rmPlaceObjectDefInArea(relicID, 0, bonusIslandID, cNonGaiaPlayers);
   
   // Pigs
   rmPlaceObjectDefInArea(farPigsID, 0, bonusIslandID, 3*cNonGaiaPlayers);


   // Bonus huntable.
   rmPlaceObjectDefInArea(bonusHuntableID, 0, bonusIslandID, 2*cNonGaiaPlayers);


   rmPlaceObjectDefInArea(bonusHuntableID2, 0, bonusIslandID, cNonGaiaPlayers);

   // Predators
   rmPlaceObjectDefInArea(farPredatorID, 0, bonusIslandID, cNonGaiaPlayers);

   // Skraeling
   rmPlaceObjectDefInArea(SkraelingID, 0, bonusIslandID, cNonGaiaPlayers);

   // Random trees
   rmPlaceObjectDefInArea(randomTreeID, 0, bonusIslandID, cNonGaiaPlayers*10);

   // Fish
   PlaceDefNearPlayers(playerFishID, false);
   ...
   rmPlaceObjectDefAtLoc(fishID, 0, 0.5, 0.5, 10*cNonGaiaPlayers);
  ...
  // Text
   ...
   rmPlaceObjectDefAtLoc(sharkID, 0, 0.5, 0.5, cNonGaiaPlayers); 
   ...
   rmPlaceObjectDefAtLoc(kelpID, 0, 0.5, 0.5, 8*cNonGaiaPlayers);
   ...
   rmPlaceObjectDefAtLoc(kelp2ID, 0, 0.5, 0.5, 12*cNonGaiaPlayers);
  ...
  // Logs
   ...
   rmPlaceObjectDefAtLoc(logID, 0, 0.5, 0.5, 2*cNonGaiaPlayers);
